...
# Change text to anything you wish, but must be 16 characters or less
    ...
    # Grab temperature info from sensor with a simple GET request
        localtempURL = "http://sensor"
        r = requests.get(localtempURL)
        datat = r.json()
        localtempF = datat['temperature'] * (9/5) + 32
        localtempC = datat['temperature']

        # Get the humidity from the DHT11 sensor running on another device
        remotehumidityURL = "http://10.128.1.131:8989/humidity"
        ...
        lcd_text("Temperature:",LCD_LINE_1)
        lcd_text('%.1f C' % localtempC,LCD_LINE_2)
